...
#define RCAR_PFC_IPSR(x)    (PFC_BASE + 0x0200 + ((x) * 4)) /* Peripheral function select */
...
static void loc_PfcSetIpsr(uint32_t IpsrIndex, uint32_t Position, uint32_t Value)
{
    uint32_t val;

    val = LOC_REG_READ32(RCAR_PFC_IPSR(IpsrIndex));
    /* All IPSR registers use a 4-bit field */
    val &= ~(0xF << Position);
    val |= (Value << Position);
    pfc_reg_write(RCAR_PFC_IPSR(IpsrIndex), val);
}
...
/*
 * These functions are for the CAN Adaptor board connected to the Salvator board
 */

static void loc_SalvatorXsCanEnableTransceiver(int channel)
{
    if (channel == 0) {
        /* For Can/Can-FD 0 */
        pfcSetGPIO(1, 12, 1);         /* _EN */
        pfcSetGPIO(1, 11, 1);         /* _STB_N */

        /* Toggle EN & STB_N to bring up the transceiver */
        gpioSetOutput(1, 12, 0);
        gpioSetOutput(1, 11, 0);

        gpioSetOutput(1, 11, 1);
        gpioSetOutput(1, 12, 1);
    } else if (channel == 1) {
        /* For Can/Can-FD 1 */
        pfcSetGPIO(1, 9, 1);    /* EN,    CN28 pin 25 => ExA9, GP1_9 */
        pfcSetGPIO(1, 8, 1);	/* STB_N, CN28 pin 27 => ExA8, GP1_8 */
        pfcSetGPIO(1, 7, 0);	/* ERR_N, CN28 pin 29 => ExA7, GP1_7 */
...
{
    /*
     * WARNING: On the Salvator-XS board with CAN Adaptor board, CAN0 pins
     * conflict with JTAG and DU.
     * Only #define ENABLE_CANFD0 if you really need it
     */
#ifdef ENABLE_CANFD0
    loc_SalvatorXsCanEnableTransceiver(0);
#endif
    loc_SalvatorXsCanEnableTransceiver(1);
}
...
void prvEbisuCanPFCInit(void)
{
    ...
    val = LOC_REG_READ32(RCAR_PFC_GPSR[0]);
    val |= BIT(12) | BIT(13);
#if RCAR_CAN_FCANCLK_EXT
    val |= BIT(14);
#endif
    pfc_reg_write(RCAR_PFC_GPSR[0], val);

    loc_PfcSetIpsr(7, 4, ipsr);     /* CAN0_TX or CANFD0_TX */
    loc_PfcSetIpsr(7, 8, ipsr);     /* CAN0_RX or CANFD0_RX */

#if RCAR_CAN_FCANCLK_EXT
    loc_PfcSetIpsr(7, 12, 1);       /* CAN_CLK */
#endif
}
...
void prvSalvatorCanPFCInit(void)
{
    ...
    val = LOC_REG_READ32(RCAR_PFC_GPSR[1]);
    val |= BIT(23) | BIT(24);
#if RCAR_CAN_FCANCLK_EXT
    val |= BIT(25);
#endif
    pfc_reg_write(RCAR_PFC_GPSR[1], val);

    loc_PfcSetIpsr(4, 24, ipsr);   /* CAN0_TX_A or CANFD0_TX_A */
    loc_PfcSetIpsr(4, 28, ipsr);   /* CAN0_RX_A or CANFD0_RX_A */
    ...
    loc_PfcSetIpsr(4, 20, ipsr);   /* CAN1_TX or CANFD1_TX */
    loc_PfcSetIpsr(5, 4, ipsr);    /* CAN1_RX or CANFD1_RX */

#if RCAR_CAN_FCANCLK_EXT
    loc_PfcSetIpsr(5, 0, 8);    /* CAN_CLK */
#endif
}
